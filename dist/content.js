const sendPage = () => {
    const structuredDataList = [
        ...
        try {
            ...
            const jsonLDDocuments = Array.isArray(jsonLD)
                ? jsonLD
                : [jsonLD];
            return jsonLDDocuments.flatMap((jsonLDDocument) => {
                const structuredDataContexts = [
                    "http://schema.org",
                    "https://schema.org",
                ];
    ...
    const articleStructuredDataImage = Array.isArray(articleStructuredDataImages) &&
    ...
    const imageURL = (ogImageElement instanceof HTMLMetaElement && ogImageElement.content) ||
    ...
    const backgroundMessage = {
        ...
        imageURL,
        metadata: [
            ...[...document.querySelectorAll("meta[name]")].flatMap((metaElement) => metaElement instanceof HTMLMetaElement
                ? [`${metaElement.name}:${metaElement.content}`]
                : []),
            ...[
                ...document.querySelectorAll('script[type="application/ld+json" i]'),
            ].flatMap((scriptElement) => scriptElement instanceof HTMLElement ? [scriptElement.innerText] : []),
        ].join(" "),
        title,
    };
...
const getArticleStructuredData = ({ structuredDataList, }) => structuredDataList.find((structuredData) => {
    const articleTypes = ["Article", "BlogPosting", "NewsArticle"];
    return articleTypes.includes(structuredData["@type"]);
});
const getBreadcrumbs = ({ structuredDataList, }) => structuredDataList
...
const getCreditLine = ({ structuredDataList, }) => {
    ...
    const credits = [
        ...new Set([
            ...
            ].flatMap((creditElement) => creditElement instanceof HTMLMetaElement
...
const getDateLine = ({ structuredDataList, }) => {
    ...
    const dateString = articleStructuredData?.dateModified ?? articleStructuredData?.datePublished;
    return (typeof dateString === "string" && new Date(dateString).toLocaleString());
};
const getDescription = ({ structuredDataList, }) => {
    ...
    return ((ogDescriptionElement instanceof HTMLMetaElement &&
...
const getHashTagLine = () => {
    ...
    const joinedKeywords = (keywordsElement instanceof HTMLMetaElement && keywordsElement.content) ||
        "";
...
const getTitle = () => {
    ...
    return ((ogTitleElement instanceof HTMLMetaElement && ogTitleElement.content) ||
        document.title);
};
const isObject = (unknown) => typeof unknown === "object" && unknown !== null;
