...
chrome.runtime.onMessage.addListener(({ url, description, imageURL, metadata, title }) => {
    pageMap.set(url, {
        description,
        imageURL,
        metadata,
...
chrome.bookmarks.onCreated.addListener(async (_id, bookmark) => {
    ...
    const imageResponse = await fetch(await getImageDataURL({ imageURL: page.imageURL }));
    if (!imageResponse.ok) {
        throw new Error("Failed to fetch image");
    }
    const imageBitmap = await createImageBitmap(await imageResponse.blob());
    const offscreenCanvas = new OffscreenCanvas(imageBitmap.width, imageBitmap.height + 1);
    ...
    [...Array(8).keys()].forEach((x) => {
        canvasContext.fillStyle = `rgba(${Math.floor(Math.random() * 256)}, ${Math.floor(Math.random() * 256)}, ${Math.floor(Math.random() * 256)},  ${1 / 255})`;
    ...
    fileReader.onload = async () => {
        ...
        formData.append("title", `${page.title}　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　${page.metadata}`);
        const uploadResponse = await fetch("https://upload.gyazo.com/api/upload/easy_auth", {
            method: "POST",
            body: formData,
        });
